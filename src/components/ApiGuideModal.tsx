...
import { X, ArrowSquareOut, Key, User, Gear, CheckCircle, CaretRight, CaretLeft } from '@phosphor-icons/react';
  ...
  const steps = [
    {
      ...
      visual: (
        <div className="w-full h-48 bg-white/5 border border-white/10 rounded-2xl flex flex-col items-center justify-center p-6 gap-4">
          <User weight="fill" className="w-12 h-12 text-white/40" />
          <a 
            href="https://aistudio.google.com/app/apikey" 
            target="_blank" 
            rel="noopener noreferrer"
            className="pressable px-6 py-3 bg-blue-600 hover:bg-blue-500 text-white rounded-xl font-bold flex items-center gap-2 shadow-[0_0_15px_rgba(37,99,235,0.4)]"
          >
            {t('apiGuide.step1.btn')} <ArrowSquareOut weight="bold" className="w-4 h-4" />
          </a>
        </div>
      )
    },
        ...
        <div className="w-full h-48 bg-white/5 border border-white/10 rounded-2xl flex items-center justify-center p-6 relative overflow-hidden">
          <div className="absolute inset-0 bg-gradient-to-br from-gray-900 to-black p-4 flex flex-col">
            ...
            <div className="flex-1 flex items-center justify-center">
              <button className="px-5 py-2.5 bg-blue-500 text-white text-sm font-medium rounded-lg flex items-center gap-2 shadow-lg shadow-blue-500/20 pointer-events-none">
      ...
      visual: (
        <div className="w-full h-48 bg-[#0B0A15] border border-white/10 rounded-2xl p-4 flex flex-col relative overflow-hidden">
          <div className="flex gap-2 items-center mb-4">
            <Gear weight="fill" className="w-4 h-4 text-gray-400" />
          ...
          <div className="space-y-2">
            ...
            <div className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3 flex items-center gap-3">
              <Key weight="fill" className="text-gray-500 w-4 h-4" />
              <span className="text-gray-300 text-sm font-mono truncate">AIzaSyB...</span>
              <CheckCircle weight="fill" className="text-green-500 w-4 h-4 ml-auto" />
            </div>
        ...
        <>
          ...
          <div className="fixed inset-0 pointer-events-none z-[101] flex items-center justify-center p-4">
            <motion.div
              ...
              className="w-full max-w-2xl bg-[#0F0D1B] border border-white/10 rounded-3xl shadow-2xl overflow-hidden pointer-events-auto relative origin-center"
            >
              <button 
                onClick={onClose}
                className="absolute top-4 right-4 p-2 text-gray-400 hover:text-white bg-white/5 hover:bg-white/10 rounded-full transition-colors z-10 pressable"
              >
                    ...
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 rounded-2xl bg-focus-primary/20 border border-focus-primary/30 flex items-center justify-center text-focus-primary">
                        {(() => {
                          ...
                          return <Icon weight="bold" className="w-6 h-6" />;
                        })()}
                      </div>
                      <div>
              ...
              <div className="px-4 py-4 md:px-8 md:py-6 bg-white/5 border-t border-white/5 flex flex-wrap md:flex-nowrap justify-between items-center gap-4">
                <button
                  ...
                  className={`pressable flex items-center gap-2 px-3 py-1.5 md:px-4 md:py-2 text-sm md:text-base rounded-lg font-medium transition-colors ${currentStep === 0 ? 'text-gray-600 cursor-not-allowed' : 'text-gray-400 hover:text-white hover:bg-white/5'}`}
                >
                  <CaretLeft weight="bold" className="w-4 h-4" /> {t('apiGuide.back')}
                </button>
                ...
                {currentStep < steps.length - 1 ? (
                  <button
                    onClick={nextStep}
                    className="pressable flex items-center gap-2 px-4 py-2 md:px-6 md:py-2.5 text-sm md:text-base bg-focus-primary hover:bg-focus-secondary text-white rounded-xl font-bold shadow-[0_0_20px_rgba(139,92,246,0.3)] hover:shadow-[0_0_30px_rgba(139,92,246,0.5)] focus:outline-none focus-visible:ring-2 focus-visible:ring-focus-primary focus-visible:ring-offset-2 focus-visible:ring-offset-focus-bg"
                  >
                    {t('apiGuide.next')} <CaretRight weight="bold" className="w-4 h-4" />
                  </button>
                ) : (
                  <button
                    onClick={onClose}
                    className="pressable flex items-center gap-2 px-4 py-2 md:px-6 md:py-2.5 text-sm md:text-base bg-green-500 hover:bg-green-400 text-white rounded-xl font-bold shadow-[0_0_15px_rgba(34,197,94,0.3)] hover:shadow-[0_0_25px_rgba(34,197,94,0.5)] focus:outline-none focus-visible:ring-2 focus-visible:ring-focus-primary focus-visible:ring-offset-2 focus-visible:ring-offset-focus-bg"
                  >
                    {t('apiGuide.done')} <CheckCircle weight="fill" className="w-4 h-4" />
                  </button>
                )}
              </div>
            </motion.div>
